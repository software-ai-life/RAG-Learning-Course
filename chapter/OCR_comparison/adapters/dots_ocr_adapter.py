...
def run_dots_ocr(
    ...
    template = os.getenv("DOTS_OCR_COMMAND")

    if not template:
        return make_failure(
            tool,
            tool_dir,
            "DOTS_OCR_COMMAND is not set. Example: DOTS_OCR_COMMAND='python /path/to/dots.ocr/demo/demo.py --input {input} --output {output_dir}'",
        )

    pages: list[dict[str, Any]] = []
    markdown_parts = ["# dots.ocr Output"]
    errors: list[str] = []
