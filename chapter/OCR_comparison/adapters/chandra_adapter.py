...
def run_chandra(
    ...
    template = os.getenv("CHANDRA_OCR_COMMAND")

    if not template:
        return make_failure(
            tool,
            tool_dir,
            "CHANDRA_OCR_COMMAND is not set. Example: CHANDRA_OCR_COMMAND='chandra {input} {output_dir}'",
        )

    pages: list[dict[str, Any]] = []
    markdown_parts = ["# Chandra Output"]
    errors: list[str] = []
