...
官方文件提供 Docker / Docker Compose 等方式啟動 Milvus。如下：
...
`query.jpg` 建議放一張和查詢文字接近的圖片，例如白色車。查詢文字可以設成：
...
{
  'auto_id': True,
  'description': '車子圖片多模態檢索',
  'fields': [
    {'name': 'id', 'type': <DataType.INT64: 5>, 'is_primary': True, 'auto_id': True},
    {'name': 'vector', 'type': <DataType.FLOAT_VECTOR: 101>, 'params': {'dim': 768}},
    {'name': 'image_path', 'type': <DataType.VARCHAR: 21>, 'params': {'max_length': 512}}
  ],
...
```python
已建立 Collection：multimodal_car_demo
{
  ...
  'functions': [],
  'aliases': [],
  'collection_id': 466290967816335710,
  'consistency_level': 2,
  ...
  'created_timestamp': 466291293279485970,
  'update_timestamp': 466291293279485970
}
```
在這個步驟中，我們先用 schema 定義 collection 的資料格式，再交給 Milvus 建立 `multimodal_car_demo` collection。這個 collection 主要包含三個欄位：`id` 作為主鍵、`vector` 儲存圖片 embedding、`image_path` 儲存原始圖片路徑。建立完成後，後續的圖片向量就會依照這個 schema 寫入 Milvus，並在下一步為 `vector` 欄位建立索引，讓相似圖片搜尋可以更有效率地執行。
...
![Milvus 多模態圖片檢索結果](../../data/C3/images/04_cars/search_result.png)
...
7. 小型教學可以先用 FAISS；正式服務或大規模資料再考慮 Milvus。
